...
const OrangeButton = ({
  ...
  <button
    ...
    onClick={onClick}
    disabled={disabled}
    className={`px-4 py-2 rounded-full font-semibold transition duration-200 text-white shadow-none hover:shadow-xl hover:shadow-[rgba(255,140,0,0.6)] disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
    style={{
...
const [isScheduling, setIsScheduling] = useState(false);
  ...
  const handleSchedulePost = async () => {
    ...
    try {
      ...
      console.log("Schedule post at:", isoString);
  
      // TODO: call your API here, e.g.
      // await fetch(`${apiUrl}/schedule-post`, { ... })
  
      setPostToast("Post scheduled successfully.");
      setTimeout(() => setPostToast(""), 3000);
      setIsScheduleOpen(false);
    } catch (e) {
      console.error(e);
      setPostToast("Failed to schedule post.");
      setTimeout(() => setPostToast(""), 3000);
    } finally {
  ...
  const handlePublishAllConnected = async () => {
    try {
      ...
      const prof = await fetch(`${apiUrl}/profile/me`, {
  ...
  return (
    <div className="min-h-screen bg-[#050816] text-white p-4 sm:p-6 lg:p-8">
  ...
  {isEditing && (
    <OrangeButton
      ...
      className={`w-full rounded-full px-4 py-2 text-sm ${
        !hasChanges || isSaving || uiDisabled
          ? "opacity-50 cursor-not-allowed"
          : ""
      }`}
    >
